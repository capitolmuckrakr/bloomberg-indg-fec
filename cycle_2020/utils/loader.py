...
logger = canon_logging.getLogger('bindg-fec.'+__name__)
...
def get_filing_list(start_date, end_date, max_fails=10, waittime=10, myextra=None):
    ...
    while True:
        ...
        try:
            ...
            if fails >= max_fails:
                if myextra:
                    myextra['TAGS']='bindg-fec, result:fail'
    ...
    if debug:
        ...
        for line in skeds:
            ...
            if line['form_type'].startswith('SE') or line['form_type'].startswith('F57'):
                ...
                if office == "P":
                    line['bindg_district'] = "PRESIDENT"
                elif office_exists and state_exists and district_exists:
                    line['bindg_district'] = "{}-{}".format(state,district)
                elif office == 'S':
                    if state_exists:
                        line['bindg_district'] = "{}-SEN".format(state)
                    else:
                        line['bindg_district'] = "UNKNOWN"
                elif state_exists:
                    if state in ['AK','DE','MT','ND','SD','VT','WY']:
                        line['bindg_district'] = "{}-AT-LARGE".format(state)
                    else:
                        line['bindg_district'] = "{}-HOUSE-UNKNOWN".format(state)
                else:
                    line['bindg_district'] = "UNKNOWN"
...
def reassign_standardized_donors(filing_id, amended_id, myextra=None):
    ...
    for transaction in matched_transactions:
        ...
        if len(new_trans) == 0:
            if myextra:
                myextra['TAGS']="bindg-fec, result:warning"
...
def load_filing(filing, filename, filing_fieldnames, myextra=None):
    ...
    if myextra:
        myextra['TAGS']="bindg-fec, result:success"
...
def load_filings(filing_dir, myextra=None):
    ...
    for filename in filing_csvs:
        ...
        try:
            ...
            if myextra:
                myextra['TAGS']="bindg-fec, result:warn"
