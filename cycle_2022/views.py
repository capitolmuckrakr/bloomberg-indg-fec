...
def get_ie_results(request):
    ...
    bindg_district = request.GET.get('bindg_district')
    min_date = request.GET.get('min_date')
    max_date = request.GET.get('max_date')
    ...
    if bindg_district:
        results = results.filter(bindg_district=bindg_district)
    if min_date:
        results = results.filter(expenditure_date__gte=min_date)
    if max_date:
        results = results.filter(expenditure_date__lte=max_date)
...
def ies(request):
    ...
    if request.GET.get('bindg_district'):
        context['bindg_district'] = True
...
def races(request):
    races = ScheduleE.objects.filter(active=True).values('bindg_district').annotate(Sum('expenditure_amount'))
    
    order_by = request.GET.get('order_by', 'expenditure_amount')
    if order_by == 'race':
        races = races.order_by('bindg_district')
    else:
